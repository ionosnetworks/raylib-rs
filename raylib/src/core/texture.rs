...
pub trait RaylibRenderTexture2D: AsRef<ffi::RenderTexture2D> + AsMut<ffi::RenderTexture2D> {
    ...
    fn depth(&self) -> &WeakTexture2D {
        unsafe { std::mem::transmute(&self.as_ref().depth) }
    }

    fn depth_mut(&mut self) -> &mut WeakTexture2D {
        unsafe { std::mem::transmute(&mut self.as_mut().depth) }
    }
}
